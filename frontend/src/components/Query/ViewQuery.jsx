...
const handleQuerySubmit = async (id) => {
  // const response = prompt("Enter your response:");
  setLoading(true); // show spinner
  try {
    
    const res=await axios.put(`http://localhost:3000/api/query/${selectedQuery._id}`,{responseMessage},{
       headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  })
...
const handleDelete=async(id)=>{
 try {
   const response = await axios.delete(`http://localhost:3000/api/query/deletePendingQuery/${id}`,
       {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")} `,
          },
  ...
  return (
    <>
    {queries.length===0?(<div className='text-center items-center text-2xl font-bold justify-center mt-50'>No Query Found</div>):(
    <div className='bg-white rounded shadow p-5 m-8'>
      <div>
        <h3 className='text-3xl font-bold text-center'>All Pending Queries</h3>
      </div>
    <table className="w-full table-auto text-sm mt-5">
      ...
      {queries.map(q => (
          <tr key={q._id} className='text-center pt-4'>
            ...
            <td>{new Date(q.forMonth).toLocaleDateString('default',{month:'long',year:'numeric'})}</td>
            <td>{q.message || "N/A"}</td>
