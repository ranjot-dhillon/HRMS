  ...
  const handleLeaveResponse = async (status) => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://localhost:3000/api/leave/responseLeave/${selectedEmployee._id}`,
      ...
      <div className="bg-white rounded shadow p-5 m-8">
        ...
        <div className="flex justify-between items-center ml-0">
          ...
          <div className="flex gap-3">
            <button onClick={()=>{filterByStatus("Approved")}} className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-semibold rounded shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
              Approved
            </button>
            <button onClick={()=>{filterByStatus("Pending")}}className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-semibold rounded shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
              Pending
            </button>

            <button onClick={()=>{filterByStatus("Rejected")}} className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-semibold rounded shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
              Rejected
            </button>
          </div>
        </div>
          ...
          <tbody>
            {filteredLeave.map((leave, index) => (
              <tr key={index} className="text-center border-t ">
                ...
                <td className="p-2">{leave.employeeId.department.dep_name}</td>
                <td className="p-2">
                  {Math.ceil(
