    ...
    <div className="h-screen fixed top-0 left-0 w-72 bg-[#E1BEE7] shadow-lg">
      ...
      <div className="flex flex-col gap-6 px-6">
        {[
          ...
          { label: 'Leaves', icon: <FaPlaneDeparture />, to: '/admin-dashboard/leaves' },
          { label: 'Salary', icon: <FaMoneyBill />, to: '/admin-dashboard/salary' },
          { label: 'Settings', icon: <FaCog />, to: '/admin-dashboard/settings' },
        ].map(({ label, icon, to }, index) => (
          <NavLink
            to={to}
            key={index}
            className={({ isActive }) =>
              `flex items-center gap-4 px-6 py-6 text-xl rounded-md border-2 transition-all font-semibold shadow-md ${
                isActive
