...
const addQuery=async(req,res)=>{
    ...
    return res.status(200).json({success:true,message:"query submitted"})
     ...
     return res.status(500).json({ success:false,message: "Internal server error in adding query" });
...
const resolveQuery=async(req,res)=>{
 try {
    ...
    return res.status(500).json({success:true,error:"in handling response"})
...
const getResolvedQueries=async(req,res)=>{
   try {
    ...
    // console.log(id)
    const resolvedQueries=await Query.find({employeeId:employee._id,status:'Resolved'}) .sort({ updatedAt: -1 })
...
const deleteQuery=async(req,res)=>{
    ...
    // console.log(id)
    const resolvedQueries=await Query.deleteMany({employeeId:employee._id,status:'Resolved'})
    return res.status(200).json({success:true});
...
export {addQuery,getAllQueries,resolveQuery,getResolvedQueries,deleteQuery,deletePendingQuery,getQueryCount}